...
import { getNeonRate, getNeonPrice } from '../pricing'
...
export function getTrackedVolumeUSD(
  ...
  const price0 = token0.derivedNEON.times(bundle.neonPrice)
  const price1 = token1.derivedNEON.times(bundle.neonPrice)
...
export function onTransfer(event: TransferEvent): void {
  ...
  // 3 cases, mints, send first on NEON withdrawls, and burns
  ...
  if (event.params.from == ADDRESS_ZERO) {
    ...
    // case where direct send first on NEON withdrawls
    const burn = new Burn(
...
export function onSync(event: SyncEvent): void {
  ...
  factory.liquidityNEON = factory.liquidityNEON.minus(pair.trackedReserveNEON as BigDecimal)
  ...
  // update NEON price now that reserves could have changed
  const bundle = getBundle()
  // Pass the block so we can get accurate price data before migration
  const neonPrice = getNeonPrice(event.block)
  bundle.neonPrice = neonPrice
  log.info('on sync block: {}, neonPrice: {}', [event.block.number.toString(), neonPrice.toString()])
  ...
  token0.derivedNEON = getNeonRate(token0Address)
  token1.derivedNEON = getNeonRate(token1Address)
  token0.save()
  token1.save()

  // get tracked liquidity - will be 0 if neither is in whitelist
  let trackedLiquidityNEON: BigDecimal
  if (bundle.neonPrice.notEqual(BIG_DECIMAL_ZERO)) {
    trackedLiquidityNEON = getTrackedLiquidityUSD(pair.reserve0, token0 as Token, pair.reserve1, token1 as Token).div(
      bundle.neonPrice
    )
  } else {
    trackedLiquidityNEON = BIG_DECIMAL_ZERO
  }

  // use derived amounts within pair
  pair.trackedReserveNEON = trackedLiquidityNEON
  pair.reserveNEON = pair.reserve0
    .times(token0.derivedNEON as BigDecimal)
    .plus(pair.reserve1.times(token1.derivedNEON as BigDecimal))
  pair.reserveUSD = pair.reserveNEON.times(bundle.neonPrice)

  // use tracked amounts globally
  factory.liquidityNEON = factory.liquidityNEON.plus(trackedLiquidityNEON)
  factory.liquidityUSD = factory.liquidityNEON.times(bundle.neonPrice)
...
export function onMint(event: MintEvent): void {
  ...
  // get new amounts of USD and NEON for tracking
  const bundle = getBundle()
  const amountTotalUSD = token1.derivedNEON
    .times(token1Amount)
    .plus(token0.derivedNEON.times(token0Amount))
    .times(bundle.neonPrice)
...
export function onSwap(event: SwapEvent): void {
  ...
  // get total amounts of derived USD and NEON for tracking
  const derivedAmountNEON = token1.derivedNEON
    .times(amount1Total)
    .plus(token0.derivedNEON.times(amount0Total))
    .div(BigDecimal.fromString('2'))
  const derivedAmountUSD = derivedAmountNEON.times(bundle.neonPrice)
  ...
  let trackedAmountNEON: BigDecimal

  if (bundle.neonPrice.equals(BIG_DECIMAL_ZERO)) {
    trackedAmountNEON = BIG_DECIMAL_ZERO
  } else {
    trackedAmountNEON = trackedAmountUSD.div(bundle.neonPrice)
  }
  ...
  factory.volumeNEON = factory.volumeNEON.plus(trackedAmountNEON)
  ...
  dayData.volumeNEON = dayData.volumeNEON.plus(trackedAmountNEON)
  ...
  token0DayData.volumeNEON = token0DayData.volumeNEON.plus(amount0Total.times(token1.derivedNEON as BigDecimal))
  token0DayData.volumeUSD = token0DayData.volumeUSD.plus(
    amount0Total.times(token0.derivedNEON as BigDecimal).times(bundle.neonPrice)
  )
  token0DayData.save()

  // swap specific updating
  token1DayData.volume = token1DayData.volume.plus(amount1Total)
  token1DayData.volumeNEON = token1DayData.volumeNEON.plus(amount1Total.times(token1.derivedNEON as BigDecimal))
  token1DayData.volumeUSD = token1DayData.volumeUSD.plus(
    amount1Total.times(token1.derivedNEON as BigDecimal).times(bundle.neonPrice)
  )
  token1DayData.save()
}
