...
import { getMoraPrice, getUSDRate } from 'pricing'
 ...
 */
export function add(event: AddPool): void {
 ...
 */
export function set(event: SetPool): void {
  const masterChef = getMasterChef(event.block)
...
export function deposit(event: Deposit): void {
  ...
  pool.accMoraPerShare = poolInfo.value3
  ...
  // Calculate MORA being paid out
  if (event.block.number.gt(MASTER_CHEF_START_BLOCK) && user.amount.gt(BIG_INT_ZERO)) {
    const pending = user.amount
      .toBigDecimal()
      .times(pool.accMoraPerShare.toBigDecimal())
      .div(BIG_DECIMAL_1E12)
      .minus(user.rewardDebt.toBigDecimal())
      .div(BIG_DECIMAL_1E18)
    if (pending.gt(BIG_DECIMAL_ZERO)) {
      const moraHarvestedUSD = pending.times(getMoraPrice(event.block))
      user.moraHarvested = user.moraHarvested.plus(pending)
      user.moraHarvestedUSD = user.moraHarvestedUSD.plus(moraHarvestedUSD)
      pool.moraHarvested = pool.moraHarvested.plus(pending)
      pool.moraHarvestedUSD = pool.moraHarvestedUSD.plus(moraHarvestedUSD)
      poolHistory.moraHarvested = pool.moraHarvested
      poolHistory.moraHarvestedUSD = pool.moraHarvestedUSD
    }
  }
...
function getMasterChef(block: ethereum.Block): MasterChef {
  ...
  if (masterChef === null) {
    ...
    // poolInfo ...
    masterChef.startTimestamp = contract.startTime()
    masterChef.mora = contract.mora()
    masterChef.moraPerSec = contract.moraPerSecond()
...
export function getPool(id: BigInt, block: ethereum.Block): Pool {
  ...
  if (pool === null) {
    ...
    pool.moraHarvested = BIG_DECIMAL_ZERO
    pool.moraHarvestedUSD = BIG_DECIMAL_ZERO
    pool.save()
  }
